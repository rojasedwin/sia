...
class Auth_model extends CI_Model {
	...
	*/
	...
	function fillSessionUserExterno($user_data,$recordar,$es_consulta){
		...
		//Session SET
		$this->session->set_userdata($user_data);
		$this->session->set_userdata('logged_inExterno', '1');
	...
	function authenticateExterno($email, $password,$codificada=0,$recordar=0){
		//Initialize return values
		$return_data = array();
		$return_data['result'] = "";
		$return_data['message'] = "";
		...
		//Search a valid username-password pair on database
		$this->db->select('*');
		$this->db->where('proveedor_email', $email);
		$this->db->where('proveedor_activo', 1);
		//$this->db->where('users.user_type !=', 1);

		$sel_user = $this->db->get('proveedores');
//echo last_query();exit;
		...
		if($sel_user->num_rows()=='1'){
			...
			if(password_verify($password,$user_id['proveedor_pwd']))
			{

				//Get User Data
				$user_data = $this->getProvDataExterno($user_id['proveedor_id']);
				//Check User Data
				if($user_data['result']=='success'){
					$return_data['result'] = 1;
					//Fill session data
					if($user_data['user_data']['proveedor_activo']==1)
						$this->fillSessionExterno($user_data['user_data'],$recordar);
					else {
						$return_data['result'] = 0;
						$return_data['message'] = "Usuario o contraseña incorrectos";
					}
					...
					//Log successfull login
					$this->logAuthenticationSuccess($user_id['proveedor_id']);
					//Set return data
					$return_data['url'] = base_url().$this->session->userdata('url_redirect');
				}
				else{
		...
		//Return data
		return $return_data;
	} //Auth Proveedor EXTERNO
	
	
	function getProvDataExterno($user_id){
		//Initialize return values
		$return_data = array();
		$return_data['result'] = "";
		$return_data['message'] = "";
		...
		//Obtenemos datos del trabajador
		$this->db->select('*');
		$this->db->where('proveedor_id', $user_id);
		$sel_datos = $this->db->get('proveedores');
		//Comprobamos los registros
		if($sel_datos->num_rows()!='1'){
			//Set return data
			$return_data['result'] = 'error';
	...
	function fillSessionExterno($user_data,$recordar){
	//Session SET
	$this->session->set_userdata($user_data);
	$this->session->set_userdata('logged_inExterno', '1');
	$this->session->set_userdata('logged_inAdmin', '0');
	...
	if($recordar!=2) delete_cookie('auth_token');  //If we don't want do anything pass 2
	if($recordar) $this->rememberMeExterno($user_data['proveedor_id']);

	$this->session->set_userdata('url_redirect',"adminsite/provexterno");

	} //Fillsession Proveedor Externo
	
	function rememberMeExterno($user_id)
	{
		$validator = $this->generatePassword();
		$hashedValidator =  password_hash($validator,PASSWORD_DEFAULT);
		$auth_token = $user_id.":".$validator;
		$upd_data = array(); $upd_data['proveedor_selectorvalidator'] = $auth_token; $upd_data['proveedor_hashedvalidator'] = $hashedValidator;
		$this->db->where("proveedor_id",$user_id)->update("proveedores",$upd_data);
		$time_cookie = time() + (10 * 365 * 24 * 60 * 60);
		$cookie = array('name'   => 'auth_token','value'  => $auth_token,'expire' => $time_cookie);
		$this->input->set_cookie($cookie);
	}
	
	function check_authTokenExterno($auth_token)
{
	$return_data = array();
	$return_data['result'] = 0;
	$tmp = explode(":",$auth_token,2);
	$user_id = $tmp[0]; $Validator = $tmp[1];
	$this->db->select('*');
	$this->db->where('proveedor_id', $user_id)->where('proveedor_selectorvalidator',$auth_token);
	$sel_user = $this->db->get('proveedores');
	...
	{
		$user_data = $sel_user->row_array();
		if( password_verify($Validator,$user_data['proveedor_hashedvalidator']))
		{
				$return_data['result'] = 1;
				//Estamos dentro le damos a recordar usuario y actualizamos

				$this->fillSessionExterno($user_data,1); //Actualizará la BBDD y la COOKIE
		}
	}
	return $return_data;
}
